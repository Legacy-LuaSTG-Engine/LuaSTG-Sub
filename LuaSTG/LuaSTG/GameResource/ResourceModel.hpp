...
namespace LuaSTGPlus
{
	...
	public:
		bool resize(uint32_t const vertex_count, uint32_t const index_count) noexcept
		{
		...
		uint32_t getVertexCount() const noexcept { return (uint32_t)vertex_.size(); }
		uint32_t getIndexCount() const noexcept { return (uint32_t)index_.size(); }
		void setAllVertexColor(Core::Color4B const color) noexcept
		{
			uint32_t const c = color.color();
			for (auto& v : vertex_) v.color = c;
		}
		void setIndex(uint32_t const index, Core::Graphics::IRenderer::DrawIndex const value) noexcept
		{
			index_[index] = value;
		}
		void setVertex(uint32_t const index, float const x, float const y, float const z, float const u, float const v, Core::Color4B const color) noexcept
		{
			vertex_[index].x = x;
			vertex_[index].y = y;
			vertex_[index].z = z;
		...
		void setVertexPosition(uint32_t const index, float const x, float const y, float const z) noexcept
		{
			vertex_[index].x = x;
			vertex_[index].y = y;
			vertex_[index].z = z;
		}
		void setVertexCoords(uint32_t const index, float const u, float const v) noexcept
		{
			vertex_[index].u = u;
			vertex_[index].v = v;
		}
		void setVertexColor(uint32_t const index, Core::Color4B const color) noexcept
		{
			vertex_[index].color = color.color();
		}
	public:
