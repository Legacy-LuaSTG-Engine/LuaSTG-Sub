...
namespace platform::windows {
	...
	private:
		...
		wil::com_ptr_nothrow<IDWriteTextFormat> dwrite_text_format_icon;
	...
	private:
		...
		void drawTitleBar() {
			...
			wil::com_ptr_nothrow<ID2D1SolidColorBrush> title_bar_background_color;
		...
		void drawButtons() {
			...
			wil::com_ptr_nothrow<ID2D1SolidColorBrush> close_press_background_color;
			d2d1_device_context->CreateSolidColorBrush(color_schema.close_button_pressed_background, close_press_background_color.put());
			wil::com_ptr_nothrow<ID2D1SolidColorBrush> close_press_text_color;
			...
			wil::com_ptr_nothrow<IDWriteTextLayout> minimize_icon_text_layout;
			dwrite_factory->CreateTextLayout(
			...
			wil::com_ptr_nothrow<IDWriteTextLayout> maximize_icon_text_layout;
			dwrite_factory->CreateTextLayout(
			...
			wil::com_ptr_nothrow<IDWriteTextLayout> close_icon_text_layout;
			dwrite_factory->CreateTextLayout(
	...
	public:
		...
		bool createResources(HWND window, ID2D1DeviceContext* context) {
			...
			initializeWindowDpi(window);
			RECT client{};
			if (!LOG_IF_WIN32_BOOL_FALSE(GetClientRect(window, &client))) {
				return false;
			}
			win32_window_width = client.right - client.left;
			win32_window_height = client.bottom - client.top;
			d2d1_device_context = context;
			hr = dwrite.CreateFactory(DWRITE_FACTORY_TYPE_SHARED, IID_PPV_ARGS(&dwrite_factory));
			if (FAILED(hr)) {
				LOG_HR_MSG(hr, "DWriteCreateFactory");
				return false;
			}
			wil::com_ptr_nothrow<IDWriteFontCollection> system_font_collection;
			hr = dwrite_factory->GetSystemFontCollection(system_font_collection.put(), false);
			if (FAILED(hr)) {
				LOG_HR_MSG(hr, "IDWriteFactory::GetSystemFontCollection");
				return false;
			}
			...
			if (FAILED(hr)) {
				LOG_HR_MSG(hr, "CreateTextFormat(\"微软雅黑\")");
				return false;
			}
			hr = dwrite_factory->CreateTextFormat(
				getIconSchema().font,
			...
			if (FAILED(hr)) {
				LOG_HR_MSG(hr, "CreateTextFormat(\"Segoe Fluent Icons\")");
				return false;
			}
			dark_mode = Platform::WindowTheme::ShouldApplicationEnableDarkMode();
			return true;
		...
		bool draw(ID2D1Bitmap1* target, D2D1_POINT_2F offset) {
			...
			if (FAILED(hr)) {
				LOG_HR_MSG(hr, "ID2D1RenderTarget::EndDraw failed");
				return false;
			}
			return true;
		}
