    ...
    if (draw_data->Textures != nullptr)
        for (ImTextureData* tex : *draw_data->Textures)
            if (tex->Status != ImTextureStatus_OK)
                if (!ImGui_ImplDX11_UpdateTexture(tex))
                    return;
...
bool ImGui_ImplDX11_UpdateTexture(ImTextureData* tex)
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        if (FAILED(bd->pd3dDevice->CreateTexture2D(&desc, &subResource, &backend_tex->pTexture)))
        {
        ...
        if (FAILED(bd->pd3dDevice->CreateShaderResourceView(backend_tex->pTexture, &srvDesc, &backend_tex->pTextureView)))
        {
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(bd->pd3dDevice->CreateVertexShader(vertex_shader_blob, sizeof(vertex_shader_blob), nullptr, &bd->pVertexShader)))
        {
        ...
        if (FAILED(bd->pd3dDevice->CreateInputLayout(local_layout, 3, vertex_shader_blob, sizeof(vertex_shader_blob), &bd->pInputLayout)))
        {
            //vertexShaderBlob->Release();
            return false;
        }
    ...
    // Create the pixel shader
    {
        ...
        //    return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
        if (FAILED(bd->pd3dDevice->CreatePixelShader(pixel_shader_blob, sizeof(pixel_shader_blob), nullptr, &bd->pPixelShader)))
        {
    ...
    // (Bilinear sampling is required by default. Set 'io.Fonts->Flags |= ImFontAtlasFlags_NoBakedLines' or 'style.AntiAliasedLinesUseTex = false' to allow point/nearest sampling)
    {
        ...
        desc.MinLOD = 0.f;
        desc.MaxLOD = 0.f;
        if (FAILED(bd->pd3dDevice->CreateSamplerState(&desc, &bd->pTexSamplerLinear)))
            return false;
        desc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
        if (FAILED(bd->pd3dDevice->CreateSamplerState(&desc, &bd->pTexSamplerNearest)))
            return false;
    }
