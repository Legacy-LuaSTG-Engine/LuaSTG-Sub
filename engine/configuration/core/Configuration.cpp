	...
	public:
		static bool merge(ConfigurationLoader& loader, std::vector<Include>* include_out, nlohmann::json const& root) {
			...
			if (root.contains("graphics_system"sv)) {
				...
				if (graphics_system.contains("disable_hardware_video_decode"sv)) {
					...
					loader.graphics_system.setDisableHardwareVideoDecode(v.get<bool>());
				}
				// TODO: display
			}
...
namespace {
	...
	const OptionMetadata meta[]{
		...
		{ .type = OptionType::boolean, .prefix = "--graphics_system.disable_hardware_video_decode="sv, .path = "/graphics_system/disable_hardware_video_decode"_json_pointer },
		// audio_system
