...
namespace LuaSTGPlus {
    ...
    void ResFX::SetViewport(fcyRect rect) noexcept {
        fcyVec4 vec(rect.a.x, rect.a.y, rect.b.x, rect.b.y);
        for (auto& p : m_pBindingViewport)
            p->SetVector(vec);
    }
    
    void ResFX::SetScreenSize(fcyVec2 size) noexcept {
    ...
    void ResFX::SetValue(const char* key, fcyColor val) noexcept {
        auto i = m_pBindingVar.find(key);
        if (i != m_pBindingVar.end()) {
    ...
    void ResFX::SetValue(const char* key, f2dTexture2D* val) noexcept {
        auto i = m_pBindingVar.find(key);
        if (i != m_pBindingVar.end()) {
            for (auto& p : i->second) {
