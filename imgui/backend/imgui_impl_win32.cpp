...
static void ImGui_ImplWin32_UpdateMouseData(ImGuiIO& io)
{
    ...
    if (is_app_focused)
    {
        ...
        if (!io.WantSetMousePos && bd->MouseTrackedArea == 0)
        {
            ...
            {
                ImVec2 transformed_pos((float)pos.x, (float)pos.y);
                if (bd->HasFrameOverride)
                {
                    transformed_pos.x += bd->MouseOffset.x;
                    transformed_pos.y += bd->MouseOffset.y;
                    transformed_pos.x *= bd->MouseScale.x;
                    transformed_pos.y *= bd->MouseScale.y;
                }
...
void    ImGui_ImplWin32_NewFrame(ImGui_ImplWin32_FrameOverride* frame_override)
{
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
    ...
    if (frame_override)
    {
    io.DisplaySize = frame_override->DisplaySize;
    bd->HasFrameOverride = true;
    bd->MouseOffset = frame_override->MouseOffset;
    bd->MouseScale = frame_override->MouseScale;
    }
    else
    {
    RECT rect = { 0, 0, 0, 0 };
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, ImGuiIO& io)
{
    ...
    case WM_NCMOUSEMOVE:
    {
        ...
        ImVec2 transformed_pos((float)mouse_pos.x, (float)mouse_pos.y);
        if (bd->HasFrameOverride)
        {
            transformed_pos.x += bd->MouseOffset.x;
            transformed_pos.y += bd->MouseOffset.y;
